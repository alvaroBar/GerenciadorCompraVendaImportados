...
class ProdutoCatalogo(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'nome': self.nome, 'preco_venda_estimado_brl': self.preco_venda_estimado_brl}
...
class ItemEstoque(db.Model):
    ...
    def to_dict(self):
        ...
        # --- ESTA É A CORREÇÃO DO BUG 1 ---
        # 'preco_venda_estimado_brl' deve vir do 'produto_info', não de 'self'
        return {
            'id': self.id, 'status': self.status, 'data_cadastro': self.data_cadastro.isoformat(),
            'produto_catalogo_id': produto_info.id,
            'nome_produto': produto_info.nome,
            'preco_venda_estimado_brl': produto_info.preco_venda_estimado_brl,  # <-- CORRIGIDO

            'preco_compra_usd': self.preco_compra_usd,
            ...
            'custo_adicional_brl': self.custo_adicional_brl,
...
# ... (Modelos Venda, Transacao, ContaAPagar permanecem iguais) ...
class Venda(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            'id_venda': self.id, 'data_venda': self.data_venda.isoformat(),
...
class Transacao(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'data': self.data.isoformat(), 'tipo': self.tipo, 'descricao': self.descricao,
...
class ContaAPagar(db.Model):
    ...
    def to_dict(self):
        return {
            'id': self.id, 'descricao': self.descricao, 'valor_brl': self.valor_brl,
            'data_vencimento': self.data_vencimento.isoformat().split('T')[0],
            'status': self.status, 'transacao_id': self.transacao_id
        }